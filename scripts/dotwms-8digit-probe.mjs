// scripts/dotwms-8digit-probe.src.ts
...
async function probeKey(email, key) {
  let res;
  try {
    res = await fetch(urlFor(email, key));
  } catch (e) {
    console.log(`    key "${key}" \u2192 transport error: ${e instanceof Error ? e.message : String(e)}`);
    return false;
  }
...
async function main() {
  ...
  if (!eights.length) {
    console.error("No 8-digit examples found in the input file.");
    process.exit(1);
  }
...
`);
  ...
  for (const e of eights) {
    ...
    const forms = [.../* @__PURE__ */ new Set([digits, `SO${digits}`])];
