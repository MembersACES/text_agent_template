...
// scripts/freight-split-finder.src.ts
var BASE = settings.machship.baseUrl;
var TOKEN = settings.machship.token;
var DELIVERED = /complete|delivered/i;
function mask(v) {
...
async function ms(method, path2, body) {
  ...
  const errs = Array.isArray(json?.errors) ? json.errors.map((e) => e?.errorMessage).filter((m) => Boolean(m)) : [];
...
async function main() {
  ...
  const from = new Date(Date.now() - days * 864e5 + 36e5).toISOString().split(".")[0];
  const to = (/* @__PURE__ */ new Date()).toISOString().split(".")[0];
  ...
  if (!multi.length) {
    console.error("No multi-consignment reference in this window. Re-run later.");
    return;
  }
  ...
  for (const [ref, v] of multi) {
    ...
    const cs = Array.isArray(look.obj) ? look.obj : [];
  ...
  evaluated.sort((a, b) => Number(b.cs.length >= 2) - Number(a.cs.length >= 2) || Number(b.mixed) - Number(a.mixed) || b.cs.length - a.cs.length);
  ...
  const full = pick.cs;
  const toEmail = full.map((c) => c.toEmail).find((e) => typeof e === "string" && e.trim().length > 0) ?? "";
  console.log(`
Rendering ${mask(pick.ref)} (lookup=${full.length}${pick.mixed ? ", MIXED status" : ""}). Simulated customer email: ${mask(toEmail)}`);
  ...
  if (!toEmail) {
    console.error("\nNo toEmail on the consignments \u2014 cannot simulate the verified customer path.");
    return;
  }
...
main().catch((e) => {
  console.error("split-finder error:", e);
  process.exitCode = 1;
});
