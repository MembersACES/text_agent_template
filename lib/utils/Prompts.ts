...
2. **EXTRACTION REQUIREMENTS**:
   ...
   - **Gas \`tariff_type\`:** Base 1 treats the bill as **unbundled** only when this field contains **"unbundled"** (case-insensitive substring). Populate **printed** tariff / product labels accordingly. Always extract **\`gas_rate_per_gj\`** (usage excl. supply ÷ GJ). **Bundled** bills then multiply that rate ×75% before comparing to the tiered benchmark. Automated gas savings rows are only emitted when annualised usage is **≥ 700 GJ/year**. The **[700, 1000) GJ** band uses the 1,000 GJ rate and is labelled Potential (C&I 70%). The benchmark ($/GJ) is otherwise tiered by annualised usage.
...
4. **BENCHMARKING & low_hanging_fruit** (utility-specific):
   ...
   - **Gas:** Deterministic Base 1 replaces gas findings.
     - Benchmark ($/GJ) tiered by annualised usage:
       ...
       - [10000, 30000): **15.0 $/GJ**
       - [30000, +inf): **13.9 $/GJ**
     - Prefer **\`gas_rate_per_gj\`** (or usage charges / ex-supply derived $/GJ).
     - **Bundled:** multiply the resolved rate ×75%, then compare to the tiered benchmark.
     - **Unbundled:** compare the resolved rate as-is (fallback: invoice ex-GST ÷ GJ).
